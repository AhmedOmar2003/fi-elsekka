...
export default function AdminPage() {
    ...
    return (
        <div className="space-y-6">
            {/* Header */}
            <div>
                <h1 className="text-2xl font-heading font-black text-white">لوحة التحكم 🛠️</h1>
                <p className="text-sm text-gray-400 mt-1">نظرة سريعة على أداء المتجر</p>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {METRIC_CARDS.map((card) => {
                    ...
                    return (
                        <Link href={card.href} key={card.label} className="group bg-[#0a0e14] border border-white/5 rounded-2xl p-5 hover:border-white/15 transition-all hover:-translate-y-0.5">
            ...
            <div className="bg-[#0a0e14] border border-white/5 rounded-2xl overflow-hidden">
                <div className="flex items-center justify-between p-5 border-b border-white/5">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="font-bold text-white text-sm">أحدث الطلبات</h2>
                    </div>
                ...
                {isLoading ? (
                    ...
                    <div className="divide-y divide-white/5">
                        {recentOrders.map((order: any) => {
                            const status = STATUS_MAP[order.status] || { label: order.status, color: 'text-gray-400 bg-white/5' };
                            return (
                                <Link href={`/admin/orders?id=${order.id}`} key={order.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-white/3 transition-colors group">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {[
                    { label: 'إضافة منتج جديد', href: '/admin/products?action=new', color: 'from-primary/20 to-transparent border-primary/20 text-primary' },
                    { label: 'إضافة قسم جديد', href: '/admin/categories?action=new', color: 'from-violet-500/20 to-transparent border-violet-500/20 text-violet-400' },
                    { label: 'إدارة المستخدمين', href: '/admin/users', color: 'from-sky-500/20 to-transparent border-sky-500/20 text-sky-400' },
                ].map((link) => (
                    <Link
                        key={link.href}
                        href={link.href}
                        className={`bg-gradient-to-br ${link.color} border rounded-2xl p-4 text-sm font-bold hover:scale-[1.02] transition-all`}
                    >
                        {link.label}
                    </Link>
                ))}
