...
import { BellRing, ChefHat, Clock3, Loader2, LogOut, PackageCheck, Store, Send, TimerReset } from "lucide-react";
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="mt-1 text-2xl font-black text-foreground">
                أهلاً {restaurant?.manager_name?.split(" ")[0] || "بيكم"}
              </h1>
        ...
        <div className="rounded-3xl border border-surface-hover bg-surface p-5">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">هنا هتلاقي الطلبات اللي فيها منتجات من مطعمك فقط.</p>
            </div>
            <button
              type="button"
              onClick={() => loadOrders()}
          ...
          {orders.length === 0 ? (
            ...
            <div className="space-y-4">
              {orders.map((order) => {
                ...
                return (
                  <div key={order.id} className="rounded-3xl border border-surface-hover bg-background/40 p-4">
                    <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                      <div>
                        ...
                        <p className="mt-1 text-xs text-gray-500">
                          اتعمل يوم {new Date(order.created_at).toLocaleDateString("ar-EG")} الساعة{" "}
                          {new Date(order.created_at).toLocaleTimeString("ar-EG", { hour: "2-digit", minute: "2-digit" })}
                        </p>
                      </div>

                      <div className="rounded-2xl border border-surface-hover bg-surface px-4 py-3 text-right">
                        <p className="text-[11px] font-black text-gray-500">إجمالي منتجات مطعمك داخل الطلب</p>
                    ...
                    <div className="mt-4 grid gap-3 md:grid-cols-2">
                      ...
                      <div className="rounded-2xl border border-surface-hover bg-surface px-4 py-3">
                        <p className="text-xs font-black text-gray-500">ملاحظة مهمة</p>
                        <p className="mt-2 text-sm leading-7 text-gray-400">
                          الإدارة تشوف وقت التوصيل اللي هتبعته من هنا، وبعد اعتمادها له هيتبلغ به العميل والمندوب.
                        </p>
                        {restaurantOrderSnapshot.restaurantName && (
                          <p className="mt-2 text-xs font-black text-primary">
                            الطلب ده ظاهر عندك كمطعم: {restaurantOrderSnapshot.restaurantName}
                          </p>
                        )}
                      </div>
                    </div>

                    <RestaurantEtaComposer
                      order={order}
